...
const SCHEDULER_ENTITIES = {
  ...
  manualStopScript: "script.irrigation_stop_zone",
...
class IrrigationMapCard extends HTMLElement {
  ...
  async stopManualZone(zone) {
    if (!this.hassObj?.states?.[SCHEDULER_ENTITIES.manualStopScript]) {
      this.notify("A frissített irrigation_scheduler.yaml még nincs betöltve.");
      return;
    }
    await this.hassObj.callService("script", "turn_on", {
      entity_id: SCHEDULER_ENTITIES.manualStopScript,
  ...
  renderScheduleEditor(container, installed) {
    ...
    container.querySelectorAll("[data-zone]").forEach((row) => {
      ...
      row.querySelector('[data-field="zone-order"]')?.addEventListener("change", (event) => {
        ...
        const other = draft.zones.find((item) => item.order === newOrder && item.id !== zone.id);
        if (other) other.order = zone.order;
        zone.order = newOrder;
        this.renderScheduler();
      });
    });
  ...
  updateScheduleStatus() {
    ...
    const timer = this.hassObj?.states?.[SCHEDULER_ENTITIES.timer];
    const remaining = timer?.state === "active" ? timerRemaining(timer) : "";
    elements.forEach((element) => {
      element.textContent = remaining ? `${status} · ${this.t("remaining")}: ${remaining}` : status;
      element.classList.toggle("running", timer?.state === "active");
    });
...
function decodeProgram(value, slot, availableZones) {
  ...
  try {
    ...
    if (new Set(zones.map((zone) => zone.order)).size !== zones.length) {
      zones.forEach((zone, index) => { zone.order = index + 1; });
    }
    return {
      slot,
...
window.customCards.push({
  type: "irrigation-map-card",
  name: "Locsolorendszer terkep",
  description: "Terkepes locsolofej es zona vezerlo kartya",
});
